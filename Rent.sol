...
*/
contract Rent is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 RENT-WFTM LP -> RENT pool
    uint256 public constant INITIAL_RENT_POOL_DISTRIBUTION = 140000 ether;
    // Distribution for airdrops wallet
    uint256 public constant INITIAL_AIRDROP_WALLET_DISTRIBUTION = 9000 ether;
    // Distribution for housing fund wallet
    uint256 public constant INITIAL_HOUSING_WALLET_DISTRIBUTION = 10000 ether;
    // Distribution for team fund wallet
    uint256 public constant INITIAL_TEAM_WALLET_DISTRIBUTION = 1000 ether;
    // Distribution for team fund wallet
    uint256 public constant INITIAL_DEV_WALLET_DISTRIBUTION = 1000 ether;
    ...
    /**
     * @notice Constructs the $RENT ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) ERC20("RENT", "RENT") {
        // Mints 1 RENT to contract creator for initial pool setup
    ...
    function _getRentPrice() internal view returns (uint256 _rentPrice) {
        try IOracle(rentOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Rent: failed to fetch $REnt price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _rentPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_rentPrice >= taxTiersTwaps[tierId]) {
    ...
    function setRentOracle(address _rentOracle) public onlyOperatorOrTaxOffice {
        require(_rentOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints $RENT to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of $RENT to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentRentPrice = _getRentPrice();
            currentTaxRate = _updateTaxRate(currentRentPrice);
            if (currentRentPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        address _housingWallet,
        address _teamWallet,
        address _devWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        require(_rentPool != address(0), "!_rentPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        require(_housingWallet != address(0), "!_housingWallet");
        require(_teamWallet != address(0), "!_teamWallet");
        require(_devWallet != address(0), "!_devWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_rentPool, INITIAL_RENT_POOL_DISTRIBUTION);
        _mint(_airdropWallet, INITIAL_AIRDROP_WALLET_DISTRIBUTION);
        _mint(_housingWallet, INITIAL_HOUSING_WALLET_DISTRIBUTION);
        _mint(_teamWallet, INITIAL_TEAM_WALLET_DISTRIBUTION);
        _mint(_devWallet, INITIAL_DEV_WALLET_DISTRIBUTION);
    }
